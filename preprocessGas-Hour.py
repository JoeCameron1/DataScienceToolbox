...
# Round the Datetimes to the nearest hour, in order to match electricity data
gas_data['Time'] = gas_data['Time'].dt.round('H')

# Sum gas data within 60 minute buckets
gas_data = gas_data.reset_index().set_index('Time')
gas_data = gas_data.resample('60T').sum()
...
gas_data.to_csv('Clean-Gas-Data-Hour/' + sys.argv[1][10:15] + '_clean.csv', columns=['Sensor Value'], index=True)
